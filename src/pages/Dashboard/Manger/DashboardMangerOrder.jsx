...
export default function DashboardRepClients() {
  ...
  const showUnApproveData = () => {
    axiosConfig
      .get(`/order/state/false`)
      .then((res) => {
        setData(res.data.data);
        setLoading(false);
      })
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.id}>
                ...
                <td>{item.quantity}</td>
                <td>
                  <FaEye onClick={() => handleShowDetails(item.id)} />
                  {approvdData && (
                    <FaCheck onClick={() => approveOrder(item.id)} />
                  )}
                </td>
              </tr>
            ))}
